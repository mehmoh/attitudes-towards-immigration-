def load_and_process(a):
    ...
    df1['age'] = 2019-df1['cps19_yob']-1919 #the variable years of birth are coded strangely. I recoded here so that the variable shows age of the respondents rather that year of birth
    df1=df1.dropna()
    df1['cps19_religion']=df1['cps19_religion'].dropna()
    ### changing numeric column to categorical


    df1['cps19_province'] = df1.cps19_province.astype('category')
    df1['cps19_religion'] = df1.cps19_religion.astype(str).astype(int)
    df1['cps19_religion'] = df1.cps19_religion.astype('category')
    df1['cps19_employment'] = df1.cps19_employment.astype('category')
    df1['cps19_gender'] = df1.cps19_gender.astype('category')
    df1['cps19_education'] = df1.cps19_education.astype('category')
    df1['cps19_imm'] = df1.cps19_imm.astype('category')
    df1['cps19_bornin_canada'] = df1.cps19_bornin_canada.astype('category')
    df1['pes19_votechoice2019'] = df1.pes19_votechoice2019.astype('category')
    df1['pes19_nativism1'] = df1.pes19_nativism1.astype('category')
    df1['pes19_nativism5'] = df1.pes19_nativism5.astype('category')
    df1['pes19_immigjobs'] = df1.pes19_immigjobs.astype('category')
    df1['pes19_immigjobs'] = df1.pes19_immigjobs.astype('category')
    df1['cps19_econ_retro'] = df1.cps19_econ_retro.astype('category')


    df1=(df1.assign(cps19_religion= lambda x: x['cps19_religion'].map({1: 'Atheist', 2: 'Agnostic', 3: 'Buddhist', 4: 'Hindu', 5: 'Jewish', 6:'Muslim',7:'Sikh', 8:'Anglican', 9:'Baptist', 10: 'Catholic',11:'Orthodox', 12:'Jehovahs Witness', 13:'Lutheran', 14:'Church of Jesus Christ of the Latter Day Saints', 15:'Pentecostal/ Fundamentalist/ Born Again/ Evangelical', 16:'Presbyterian', 17:'Protestant', 18:'United Church of Canada', 19:'Christian Reformed', 20:'Salvation Army', 21:'Mennonite', 22:'Other'}))
